...
assert(type(MenuEnvironment.AutoBountyConfig) == "table",
    "Set AutoBountyConfig before loading the menu version.")

do
    local oldMenu = MenuEnvironment.__AutoBountyMenu
    if type(oldMenu) == "table" and type(oldMenu.Stop) == "function" then
        pcall(oldMenu.Stop, oldMenu, "reload")
    end
    local oldRuntime = MenuEnvironment.__AutoBountyRuntime
    if type(oldRuntime) == "table" and type(oldRuntime.Stop) == "function" then
        pcall(oldRuntime.Stop, oldRuntime, "menu loaded")
    end
    MenuEnvironment.__AutoBountyBootstrapToken = {}
    local oldBoost = MenuEnvironment.__AutoBountyFPSBoost
    if type(oldBoost) == "table" and type(oldBoost.Stop) == "function" then
        pcall(oldBoost.Stop, oldBoost)
    end
end

local Menu = {
    Active = true, Running = true, Open = true,
    HuntEnabled = false, AutoHopEnabled = false,
    SelectedTeam = MenuEnvironment.AutoBountyConfig.Team == "Marines" and "Marines" or "Pirates",
    TeamBusy = false, TeamEpoch = 0, Connections = {}, Controls = {},
    Status = "Ready. Choose a team, then start bounty hunt.",
}
MenuEnvironment.__AutoBountyMenu = Menu

function Menu:IsCurrent()
    return self.Active and MenuEnvironment.__AutoBountyMenu == self
end

function Menu:SetSessionStatus(session, status)
    if self:IsCurrent() and self.Session == session and not session.Cancelled then
        self.Status = status
        self:Refresh()
    end
end

function Menu:StopSession()
    ...
    if session.Runtime then
        if type(session.Runtime.Stop) == "function" then
            pcall(session.Runtime.Stop, session.Runtime, "menu stop")
        else
            session.Runtime.Running = false
        end
    end
    if session.BootstrapToken and MenuEnvironment.__AutoBountyBootstrapToken == session.BootstrapToken then
        MenuEnvironment.__AutoBountyBootstrapToken = {}
    end
    if session.FPSBoost and type(session.FPSBoost.Stop) == "function" then
        pcall(session.FPSBoost.Stop, session.FPSBoost)
    end
end
...
function Menu:SyncSession()
    ...
    for key, value in pairs(source) do config[key] = value end
    for key, value in pairs(type(source.Settings) == "table" and source.Settings or {}) do
        settings[key] = value
    end
    config.Team = self.SelectedTeam
    ...
    task.spawn(function()
        local ok, result = pcall(self.RunSession, session)
        if not self:IsCurrent() or self.Session ~= session or session.Cancelled then
            if ok and type(result) == "table" and type(result.Stop) == "function" then
                pcall(result.Stop, result, "cancelled startup")
            end
            return
        end
...
function Menu:SelectTeam(team)
    ...
    self.TeamEpoch = self.TeamEpoch + 1
    local epoch = self.TeamEpoch
    self.TeamBusy = true
    self.Status = "Joining " .. team .. "..."
    self:Refresh()
    task.spawn(function()
        ...
        while current() and os.clock() < deadline do
            if lastRequest and lastRequest.FinishedAt then
                retryAt = lastRequest.FinishedAt + 5
                lastRequest = nil
            end
            if not MenuEnvironment.__AutoBountyTeamRequest
                and self.Player.Team and self.Player.Team.Name == team then
                confirmed = true
                break
            end
            if not MenuEnvironment.__AutoBountyTeamRequest and os.clock() >= retryAt then
                local remotes = MenuReplicatedStorage:FindFirstChild("Remotes")
                local remote = remotes and remotes:FindFirstChild("CommF_")
                if remote and remote:IsA("RemoteFunction") then
                    local request = {Remote = remote}
                    lastRequest = request
                    MenuEnvironment.__AutoBountyTeamRequest = request
                    task.spawn(function()
                        -- Keep the shared lock until the actual call returns, even if this selection is cancelled.
                        pcall(function() remote:InvokeServer("SetTeam", team) end)
                        request.FinishedAt = os.clock()
                        if MenuEnvironment.__AutoBountyTeamRequest == request then
                            MenuEnvironment.__AutoBountyTeamRequest = nil
                        end
                    end)
                end
            end
        ...
        self.TeamBusy = false
        self.Status = confirmed and (team .. " selected. Ready to start bounty hunt.")
            or "Team change not confirmed. Select a team again to retry."
        self:Refresh()
    end)
    return true
end
...
function Menu:Stop(reason)
    ...
    if MenuEnvironment.__AutoBountyMenu == self then MenuEnvironment.__AutoBountyMenu = nil end
end

function Menu:CreateGUI()
    local playerGui = self.Player and self.Player:FindFirstChildOfClass("PlayerGui")

    if not self.Active or not playerGui then
        return false
    end
    ...
    local function decorate(object, radius, strokeColor)
    ...
    local function button(name, text, position, size, parent)
        local object = create("TextButton", {
            ...
            Position = position,
            Size = size,
            BackgroundColor3 = Color3.fromRGB(40, 45, 57),
            BorderSizePixel = 0,
            TextColor3 = Color3.fromRGB(237, 240, 247),
            Font = Enum.Font.GothamMedium,
            TextSize = 14,
            AutoButtonColor = false,
        }, parent)

        decorate(object, 8, Color3.fromRGB(81, 90, 110))
        return object
    end

    local function connect(object, callback)
        self.Connections[#self.Connections + 1] = object.Activated:Connect(function()
            if self.Active then
                callback()
            end
        end)
    ...
    local toggle = button("ToggleMenu", "Hide menu", UDim2.fromOffset(12, 12), UDim2.fromOffset(124, 40), gui)
    local panel = create("Frame", {
        Name = "Panel",
        Position = UDim2.fromOffset(12, 60),
        Size = UDim2.fromOffset(300, 292),
        BackgroundColor3 = Color3.fromRGB(22, 26, 35),
        BorderSizePixel = 0,
    }, gui)
    decorate(panel, 12, Color3.fromRGB(87, 103, 130))
    ...
    create("TextLabel", {
        ...
        Text = "Bounty Hunt",
        TextColor3 = Color3.fromRGB(244, 246, 251),
    ...
    local teamLabel = create("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Text = "Choose team",
        TextColor3 = Color3.fromRGB(165, 177, 199),
        Font = Enum.Font.Gotham,
        TextSize = 12,
        TextXAlignment = Enum.TextXAlignment.Left,
    }, panel)

    local pirates = button("Pirates", "Pirates", UDim2.fromOffset(16, 66), UDim2.fromOffset(130, 40), panel)
    local marines = button("Marines", "Marines", UDim2.fromOffset(154, 66), UDim2.fromOffset(130, 40), panel)
    local hop = button("ServerHop", "Server hop: OFF", UDim2.fromOffset(16, 116), UDim2.fromOffset(268, 42), panel)
    local hunt = button("BountyHunt", "Start bounty hunt", UDim2.fromOffset(16, 166), UDim2.fromOffset(268, 44), panel)
    local status = create("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Text = "",
        TextColor3 = Color3.fromRGB(174, 186, 207),
        Font = Enum.Font.Gotham,
        TextSize = 12,
        TextWrapped = true,
        TextXAlignment = Enum.TextXAlignment.Left,
    ...
    connect(pirates, function()
        if not self.TeamBusy then
            self:SelectTeam("Pirates")
        end
    end)
    connect(marines, function()
        if not self.TeamBusy then
            self:SelectTeam("Marines")
        end
    end)
    connect(hop, function()
        self:SetAutoHopEnabled(not self.AutoHopEnabled)
    end)
    connect(hunt, function()
        if not self.TeamBusy or self.HuntEnabled then
            self:SetHuntEnabled(not self.HuntEnabled)
        end
    end)
...
function Menu:Refresh()
    ...
    if not controls or not self.GUI or not self.GUI.Parent then
        return
    end

    local inactive = Color3.fromRGB(40, 45, 57)
    local selected = Color3.fromRGB(46, 94, 159)
    local enabled = Color3.fromRGB(34, 112, 86)
    local stopped = Color3.fromRGB(145, 56, 65)
    local active = self.Active == true
    local choosingTeam = active and not self.TeamBusy
    local canToggleHunt = active and (not self.TeamBusy or self.HuntEnabled)

    controls.Panel.Visible = self.Open == true
    controls.Toggle.Text = self.Open and "Hide menu" or "Menu"
    controls.Toggle.Active = active

    controls.TeamLabel.Text = self.TeamBusy and "Switching team..." or "Choose team"
    controls.Pirates.BackgroundColor3 = self.SelectedTeam == "Pirates" and selected or inactive
    controls.Marines.BackgroundColor3 = self.SelectedTeam == "Marines" and selected or inactive
    controls.Pirates.Active = choosingTeam
    controls.Marines.Active = choosingTeam
    controls.Pirates.TextTransparency = choosingTeam and 0 or 0.4
    controls.Marines.TextTransparency = choosingTeam and 0 or 0.4

    controls.Hop.Text = self.AutoHopEnabled and "Server hop: ON" or "Server hop: OFF"
    controls.Hop.BackgroundColor3 = self.AutoHopEnabled and enabled or inactive
    controls.Hop.Active = active
    controls.Hop.TextTransparency = active and 0 or 0.4

    controls.Hunt.Text = self.HuntEnabled and "Stop bounty hunt" or "Start bounty hunt"
    controls.Hunt.BackgroundColor3 = self.HuntEnabled and stopped or enabled
    controls.Hunt.Active = canToggleHunt
    controls.Hunt.TextTransparency = canToggleHunt and 0 or 0.4

    controls.Status.Text = tostring(self.Status or "Bounty hunt is stopped. Server hop is off.")
end

function Menu.RunSession(MenuSession)
if MenuSession.Cancelled or not MenuSession.Owner:IsCurrent() then return end
...
local Environment = getgenv and getgenv() or _G
local Config = MenuSession.Config
local LoaderToken = Environment.__AutoBountyLoaderToken
local BootstrapToken = {}
MenuSession.BootstrapToken = BootstrapToken
MenuSession.Owner:SetSessionStatus(MenuSession, "Joining " .. Config.Team .. "...")
...
local function bootstrapStillCurrent()
    return not MenuSession.Cancelled and MenuSession.Owner:IsCurrent()
...
MenuSession.FPSBoost = StartupFPSBoost
do
...
MenuSession.Owner:SetSessionStatus(MenuSession, "Waiting for player data...")
...
-- Live menu control: every hop gate shares this local flag.
function Runtime:SetAutoHopEnabled(enabled)
    if not self.Running or not bootstrapStillCurrent()
        or Environment.__AutoBountyRuntime ~= self then
        return false
    end
    enabled = enabled == true
    AutoHopEnabled = enabled
    Settings.AutoHop = enabled
    self.AutoHopEnabled = enabled
    if not enabled then
        if self.HopPending or self.EmptyHopCommitted or self.HopSearch or self.HopAttempt then
            stopHopPending("Server hop off; scanning")
        end
    else
        local reason, detail = determineHopReason()
        if reason then requestHop(reason, detail) end
    end
    return true
end
...
if not bootstrapStillCurrent() then
    Runtime:Stop("cancelled startup")
    return
end
MenuSession.Ready = true
Runtime:SetAutoHopEnabled(MenuSession.HopEnabled)
return Runtime

end

while Menu:IsCurrent() and not game:IsLoaded() do task.wait(0.1) end
if not Menu:IsCurrent() then return Menu end
Menu.Player = MenuPlayers.LocalPlayer
while Menu:IsCurrent() and not Menu.Player do
    task.wait(0.1)
    Menu.Player = MenuPlayers.LocalPlayer
end
while Menu:IsCurrent() and not Menu.Player:FindFirstChildOfClass("PlayerGui") do task.wait(0.1) end
if not Menu:IsCurrent() then return Menu end
Menu:CreateGUI()
return Menu
